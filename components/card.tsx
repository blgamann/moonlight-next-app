"use client";

import React, { useRef, useEffect, useState } from "react";
import { TextDarkGrey } from "./text";

export function CardLeftLine({ children }: { children: React.ReactNode }) {
  return (
    <div className="max-w-[450px] relative pl-9 pr-6 py-6.5 border-[0.75px] border-gray-300 border-l-0 shadow-[0px_7px_25px_0px_rgba(0,0,0,0.04)]">
      <div
        className="
          absolute
          left-0
          top-0
          bottom-0
          w-[3.5px]
          bg-[linear-gradient(178.8deg,_#56c1ff,_#6ae8d8)]
        "
      />
      {children}
    </div>
  );
}

export function CardTopLine({ children }: { children: React.ReactNode }) {
  return (
    <div className="max-w-[450px] relative py-6.5 px-6 border-[0.75px] border-gray-300 border-t-0 shadow-[0px_7px_25px_0px_rgba(0,0,0,0.04)]">
      <div
        className="
          absolute
          left-0
          right-0
          top-0
          h-[3.5px]
          bg-[linear-gradient(178.8deg,_#6ae8d8,_#56c1ff)]
        "
      />
      {children}
    </div>
  );
}

export function CardWaiting({ children }: { children: React.ReactNode }) {
  const strokeWidth = 3.5;
  const rectRef = useRef<SVGRectElement>(null);

  const [perimeter, setPerimeter] = useState(0);
  const [segment, setSegment] = useState(0);

  useEffect(() => {
    if (!rectRef.current) return;
    const path = rectRef.current;
    const full = path.getTotalLength(); // path 전체 둘레
    const { height } = path.getBBox(); // 세그먼트 길이 (사이드 높이)
    setPerimeter(full);
    setSegment(height);
  }, []);

  const gap = Math.max(0, perimeter - segment);

  return (
    <div className="relative max-w-[450px] border-[0.75px] border-gray-300 shadow-[0px_7px_25px_0px_rgba(0,0,0,0.04)]">
      <svg
        className="absolute inset-0 w-full h-full pointer-events-none z-10"
        preserveAspectRatio="none"
      >
        <defs>
          <linearGradient id="borderGrad" gradientTransform="rotate(178.8)">
            <stop offset="0%" stopColor="#6ae8d8" />
            <stop offset="100%" stopColor="#56c1ff" />
          </linearGradient>
        </defs>
        <rect
          ref={rectRef}
          x={strokeWidth / 2}
          y={strokeWidth / 2}
          width="calc(100% - 3.5px)"
          height="calc(100% - 3.5px)"
          fill="none"
          stroke="url(#borderGrad)"
          strokeWidth={strokeWidth}
          style={{
            strokeDasharray: `${segment} ${gap}`, // [세그먼트, 갭]
            strokeDashoffset: 0,
            animation: "moveSegment 8s linear infinite",
          }}
        />
      </svg>
      <div className="relative z-0 py-6.5 px-6 bg-white border-t-0 shadow-[0px_7px_25px_0px_rgba(0,0,0,0.04)]">
        {children}
      </div>
      <style jsx global>{`
        @keyframes moveSegment {
          to {
            stroke-dashoffset: -${perimeter}px;
          }
        }
      `}</style>
    </div>
  );
}

export function CardComponent() {
  const content = (
    <TextDarkGrey>
      {
        "죽음은 신나게 놀고 있는데 엄마가 '얘야, 그만 놀고 들어와 밥 먹어라'하고 부르는 소리와 같습니다."
      }
    </TextDarkGrey>
  );

  return (
    <div className="flex flex-col gap-4">
      <CardLeftLine>{content}</CardLeftLine>
      <CardTopLine>{content}</CardTopLine>
      <CardWaiting>{content}</CardWaiting>
    </div>
  );
}
